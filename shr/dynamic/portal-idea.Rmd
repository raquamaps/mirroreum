---
title: "Reproducible Open Research Analysis Portal"
output: 
  html_document: 
    theme: spacelab
---

This is an outline for a possible subsection of the EUBON Portal. It is based on the idea of having two different separate sub-areas as separate sections of the larger portal - probably with the same look and feel if that is possible to acheive without too much customizations.

1. The first one is a regular simple and straightforward channel that communicates with _*policy makers*_ and regular users - essentially like a easily readable and interesting blog (like Wordpress, Ghost or similar). 
...
Use a suitable hostname (suggestions?) to brand the service directed towards data scientists. This web-friendly Analysis Portal for data scientist would feature:

  - Docker and docker-compose for packaging and deployment 
...
### Packaging approach
  ...
  - Push the integration project to GitHub and then deploy to production server by cloning that github repo and starting the app with "docker-compose". Or push an image to a Docker registry such as the public Docker Hub (requires a little bit more work).  (NOT DONE)
